...
public class Sample_Pool : MonoBehaviour {
    ...
    [Header ("测试")]
    [Header ("通过IMGUI 模拟触发Cube测试对象的生成与销毁")]
    ...
    void OnGUI () {
        ...
        if (GUI.Button (new Rect (10, 10, 160, 40), "Get"))
            Get_One ();
        if (GUI.Button (new Rect (180, 10, 160, 40), "Release One"))
            Release_One ();
        if (GUI.Button (new Rect (360, 10, 160, 40), "Release All"))
            Release_All ();
        if (GUI.Button (new Rect (540, 10, 160, 40), "Destroy Pool")) {
